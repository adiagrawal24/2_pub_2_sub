...
class MyPublisher(Node):
    def __init__(self):
        super().__init__('first_publisher')  # Initialize the Node with the name 'my_publisher'
        self.publisher_ = self.create_publisher(String, 'topic1', 10)  # Create a publisher
    ...
    def timer_callback(self):
        ...
        msg.data = 'Mesaage 1 Going'  # Assign data to the message
...
def main(args=None):
    ...
    first_publisher = MyPublisher()  # Create a MyPublisher object
    rclpy.spin(first_publisher)  # Keep the node alive to continue publishing

    first_publisher.destroy_node()  # Cleanup the node
